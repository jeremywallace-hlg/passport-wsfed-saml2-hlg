...
const forge = require('node-forge'); // added when removing x509 // const x509 = require('x509');
...
SAML.prototype.extractAndValidateCertExpiration = function (validatedSamlAssertion, optionsCert) {
  ...
  const parsedCert = forge.pki.certificateFromPem(utils.certToPEM(cert)); // const parsedCert = x509.parseCert(utils.certToPEM(cert));

  const nowDate = new Date();

  // true if current date is before expiry AND after cert start date
  if ( ! (nowDate > parsedCert.validity.notBefore && nowDate < parsedCert.validity.notAfter)) {   // if ( ! (nowDate > parsedCert.notBefore && nowDate < parsedCert.notAfter)) {
